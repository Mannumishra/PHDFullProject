...
const AddMultiUniversities = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!universityZone || universities.some((uni) => !uni)) {
            Swal.fire('Error', 'Please select a University Zone and provide university names', 'error');
            return;
        }
        ...
        try {
            ...
            setUniversityZone('');
            setUniversities(['']);
            navigate('/add-university'); // Redirect to the universities list page
        } catch (error) {
    ...
    return (
        <>
            <div className="bread">
                ...
                <div className="links">
                    <Link to="/all-university" className="add-new">Back <i className="fa-regular fa-circle-left"></i></Link>
                </div>
            </div>
