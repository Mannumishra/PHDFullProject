...
const InqueryFrom = () => {
    ...
    useEffect(() => {
        ...
        const fetchRequests = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/all-consultation');
    ...
    const deleteRecord = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await axios.delete(`http://localhost:8000/api/delete-consultation/${id}`);
