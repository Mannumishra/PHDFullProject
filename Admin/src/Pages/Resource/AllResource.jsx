...
const AllResource = () => {
    ...
    useEffect(() => {
        const fetchResources = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/get-resources'); // Get resources from your backend API
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                await axios.delete(`http://localhost:8000/api/delete-resources/${id}`);
                        ...
                        {resources.length === 0 ? (
                            ...
                            resources.map((resource, index) => (
                                <tr key={resource._id}>
                                    ...
                                    <td>
                                        <a href={`http://localhost:8000/${resource.ResourcesPdf}`} target="_blank" rel="noopener noreferrer">
                                            View PDF
                                        </a>
                                    </td>
