...
const AllProfession = () => {
    ...
    const getApiData = async () => {
        try {
            const res = await axios.get("http://localhost:8000/api/all-profession-guide"); // Update the endpoint
            if (res.status === 200) {
    ...
    const deleteProfessional = async (id) => {
        try {
            ...
            if (result.isConfirmed) {
                const res = await axios.delete(`http://localhost:8000/api/delete-profession-guide/${id}`);
                if (res.status === 200) {
