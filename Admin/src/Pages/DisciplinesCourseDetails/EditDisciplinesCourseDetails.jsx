...
const EditDisciplinesCourseDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const disciplinesResponse = await axios.get('http://localhost:8000/api/all-discipline');
        const courseNamesResponse = await axios.get('http://localhost:8000/api/all-disciplines-course-make-product');
        ...
        const courseResponse = await axios.get(`http://localhost:8000/api/single-disciplines-course-details/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await axios.put(`http://localhost:8000/api/update-disciplines-course-details/${id}`, formData);
