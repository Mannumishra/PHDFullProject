...
const AllDisciplines = () => {
    ...
    const fetchDisciplines = async () => {
        try {
            const response = await axios.get('http://localhost:8000/api/all-discipline'); // Ensure this is the correct endpoint
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (result.isConfirmed) {
                const response = await axios.delete(`http://localhost:8000/api/delete-discipline/${id}`); // Ensure this is the correct delete endpoint
    ...
    const updateStatus = async (id, newStatus) => {
        try {
            const response = await axios.put(`http://localhost:8000/api/update-discipline/${id}`, {
                    ...
                    <tbody>
                        {disciplines.map((discipline, index) => (
                            <tr key={discipline._id}>
                                ...
                                <td>
                                    <img src={`http://localhost:8000/${discipline.DisciplinesLogo}`} alt={discipline.DisciplinesName} width="50" height="50" />
                                </td>
                                <td>
                                    <input
                                        type="checkbox"
