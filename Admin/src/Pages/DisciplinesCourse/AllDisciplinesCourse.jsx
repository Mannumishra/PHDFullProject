...
const AllDisciplinesCourse = () => {
    ...
    useEffect(() => {
        const fetchDisciplineCourses = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/all-disciplines-course'); // Adjust endpoint
    ...
    const handleDelete = async (id) => {
        ...
        if (confirm.isConfirmed) {
            try {
                await axios.delete(`http://localhost:8000/api/delete-disciplines-course/${id}`); // Adjust endpoint
